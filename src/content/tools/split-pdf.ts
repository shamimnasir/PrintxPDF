...
export const splitPdf: ToolContent = {
  slug: 'split-pdf',
  answer:
    'Split PDF breaks one PDF into several smaller files: by custom page ranges, every N pages, or one file per page. It runs in your browser, so the document is never uploaded. Download the pieces separately or as one ZIP folder, free and without an account.',
  whatHeading: 'What does splitting a PDF do?',
  what: [
    {
      term: 'How a PDF split works',
      definition:
        'Each output is a brand-new PDF that contains **copies of the chosen pages** from the source. The pages travel with their fonts, pictures and drawings, so nothing is redrawn and quality is unchanged. The original file is not changed. Output files are named after the source with the page span added, so `report.pdf` split at pages 1 to 3 becomes `report-p1-3.pdf`.',
    },
    {
      term: 'What a page range is',
      definition:
        'A range is written as a start and an end page joined by a hyphen, counting the first page in the file as 1: `1-3` is pages one to three, `5` is a single page and `7-` runs from page seven to the end. Separate ranges with commas and each one becomes its own file. Pages you leave out of every range are simply not included in any output.',
    },
  ],
  ...
  why: [
    ...
    { h: 'Stay under attachment limits', x: 'Most mail services cap attachments at around 20 to 25 MB. Splitting a large scan into halves gets it through without lowering picture quality.' },
    { h: 'Nothing is redrawn', x: 'Pages are copied, not turned into pictures, so the split files look and search exactly like the original.' },
    ...
    { h: 'All the pieces in one download', x: 'A split into forty single pages does not mean forty clicks. Download ZIP collects every output into one folder.' },
  ],
  ...
  how: [
    ...
    { h: 'Download the parts.', x: 'Every output appears in the Ready list. Use Download ZIP to get all of them in one folder, or download each file on its own. Names carry the page span, such as `invoice-p4-6.pdf`.' },
  ],
  faqs: [
    {
      q: 'Can I split a PDF into individual pages?',
      a: 'Yes. Choose Every page under Split by and run. Each page becomes its own PDF, named with its page number, and Download ZIP collects them all in one folder. For a document of a few hundred pages this takes a few seconds in a desktop browser.',
    },
    {
      q: 'Does splitting a PDF reduce quality?',
      a: 'No. Every page is copied into the new file together with its fonts and pictures. Nothing is turned into a picture or recompressed, so text stays selectable and pictures keep their detail. The only things that do not carry over are extras that belong to the whole file rather than to a page, such as bookmarks.',
    },
    {
      q: 'Can I pull out just one section instead of splitting everything?',
      a: 'Yes. Enter a single range such as `12-18` under Custom ranges and you get one file with those pages. If you want several scattered pages in one output rather than one file per range, use [Extract Pages](/tools/extract-pages) instead, which accepts a list like `1, 4, 9-10`.',
    },
    {
  ...
  entities: ['PDF', 'ISO 32000', 'Page range', 'ZIP archive', 'Adobe Acrobat', 'macOS Preview'],
